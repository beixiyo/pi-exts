 ...
 * - 无 options → 同一组件的纯输入模式（pi 内置 ui.input 不支持 ctrl+v 贴图，
 *   统一走本组件保证粘贴行为与主输入框一致）；有 options → 组合对话框（见 ./dialog.ts）
...
const parameters = Type.Object({
  questions: Type.Array(Type.Object(singleQuestion), {
    description:
      'Questions to ask in sequence — one dialog per question; a single-element array asks one question. Prefer batching related questions into one call instead of repeated tool calls',
  }),
})
...
export const askUserTool: ToolDefinition<typeof parameters> = {
  ...
  promptSnippet:
    'When you need information only the user knows, ask via the ask_user dialog tool — batch related questions into one call using the `questions` array.',
  parameters,
    ...
    for (const [index, q] of list.entries()) {
      ...
      if (answer === undefined) {
        ...
        return {
          content: [{
            type: 'text',
            text: `The user canceled at question ${
              index + 1
            } of ${list.length}.${collected} Do not retry the dialogs; ask the remaining questions in plain text.`,
          }],
          isError: true,
          details: undefined,
        }
