...
| 扩展 | 提供 | 功能 |
|------|------|------|
| `ask-user` | `ask_user` 工具 | 弹窗向用户提问——选项列表 + 自定义答案输入框，单选/多选 |
| `yank` | `/yank` | 从会话历史中精确选择回复/代码块/问题并复制到剪贴板 |
| `rename` | `/rename` | 首轮对话后自动命名会话（可用低成本模型）；`/rename <标题>` 手动命名 |
| `status` | `/status` | 会话信息卡片（会话文件、恢复命令、工作目录、模型、上下文用量）——同时复制接力文本到剪贴板 |
| `statusline` | footer | 彩色状态栏：模型、思考级别、git 分支、扩展状态、上下文用量、厂商配额 |
| `slash-anywhere` | 补全 | 句中触发 `/命令` 与 skill 补全（`$cmd` 自动、`/cmd` + Tab 手动） |
| `md-code-bg` | 渲染 | markdown 行内代码背景色块——opencode 风格 |

所有配置都在 pi 的 `settings.json` 中读取，**全部可省略**——每个扩展都有合理默认值
...
从会话中挑选内容复制——一字不差。三种模式 Tab 循环切换：**answers**（每轮回复，可展开出其中的代码块）、**codes**（全部代码块平铺）、**questions**（每轮用户消息）。直接输入即模糊过滤，↑↓ 移动，enter 复制
...
```jsonc
// settings.json —— 可省略；缺省跟随 pi 默认模型
{
  "autoRename": {
...
在聊天记录中追加一张持久卡片（不进 LLM 上下文）：会话名、Session ID、会话文件、可直接粘贴的恢复命令、工作目录、git 分支、模型、上下文用量
...
markdown 行内 `` `代码` `` 以背景色块渲染（opencode 风格）。fenced 代码块不受影响
